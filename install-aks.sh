...
EOF
  ...
  echo && read -p "Would you like to use the experimental branch? It contains the last major changes, such as the new browser and migrating to Docker (y/N) ABSOLUTELT NOT!!" -n 1 -r -s EXP && echo
  if [ "$EXP" != 'y'  ]; then
    echo && read -p "Would you like to use the development (master) branch? You will get the latest features, but things may break. (y/N) YES!!" -n 1 -r -s DEV && echo
    if [ "$DEV" != 'y'  ]; then
  ...
  echo && read -p "Would you like to install the WoTT agent to help you manage security of your Raspberry Pi? (y/N) YES!!" -n 1 -r -s WOTT && echo
  if [ "$WOTT" = 'y' ]; then
