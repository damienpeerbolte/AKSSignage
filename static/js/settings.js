...
(function() {
  $().ready(function() {
    ...
    $("#btn-backup").click(function(e) {
      ...
      return $.ajax({
        ...
        if ((data.responseText !== "") && (j = $.parseJSON(data.responseText)) && (err = j.error)) {
          ...
          return ($("#request-error .msg")).text("Er is iets fout gegaan. Herlaad de pagina en probeer het opnieuw.");
        }
    ...
    $("#btn-reset").click(function(e) {
      if (confirm("Weet u zeker dat u de Wi-Fi configuratie wilt resetten?")) {
        return $.get("/api/v1/reset_wifi").done(function(e) {
          ...
          $("#request-error .alert").addClass("alert-success");
          $("#request-error .alert").removeClass("alert-danger");
          return ($("#request-error .msg")).text("De Wi-Fi is gereset! Herstart het apparaat.");
        }).error(function(e) {
        ...
        getStatus = function(id) {
          return $.get("/upgrade_status/" + id).done(function(data, e, jqXHR) {
            ...
            if ((data.responseText !== "") && (j = $.parseJSON(data.responseText)) && (err = j.error)) {
              ...
              return ($("#upgrade_logs")).append("Er is iets fout gegaan. Herlaad de pagina en probeer het opnieuw.");
            }
          });
        };
    ...
    $("#btn-reboot-system").click(function(e) {
      if (confirm("Weet u zeker dat u AKSSignage wilt herstarten?")) {
        return $.post("/api/v1/reboot_screenly").done(function(e) {
          ...
          ($("#request-error .alert")).addClass("alert-success");
          ($("#request-error .alert")).removeClass("alert-danger");
          return ($("#request-error .msg")).text("AKSSignage is aan het herstarten!");
        }).fail(function(data, e) {
          var err, j;
          ($("#request-error .alert")).show();
          ($("#request-error .alert")).addClass("alert-danger");
    ...
    $("#btn-shutdown-system").click(function(e) {
      if (confirm("Weet u zeker dat u AKSSignage wilt afsluiten?")) {
        return $.post("/api/v1/shutdown_screenly").done(function(e) {
          ($("#request-error .alert")).show();
          ($("#request-error .alert")).addClass("alert-success");
          ($("#request-error .alert")).removeClass("alert-danger");
          return ($("#request-error .msg")).text("AKSSignage aan het uitzetten... U kunt zo het apparaat van de stroom halen!");
        }).fail(function(data, e) {
          var err, j;
          ($("#request-error .alert")).show();
          ($("#request-error .alert")).addClass("alert-danger");
